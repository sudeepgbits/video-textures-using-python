...
def runTexture(img_list, alpha):
    ...
    #diff3_2 = vizDifference(diff3)
    #print(idxs)
    [start, end] = idxs 
    center = int(0.5 * start + 0.5 * end)
    radius = int((end - start) * 0.5)
    #diff3 = cv2.circle(diff3_2,(center,center), 1, (0,1), -1)
    return (vizDifference(ssd_diff),
            vizDifference(transition_diff),
            vizDifference(diff3),
            textures.synthesizeLoop(video_volume, idxs[0], idxs[1]), idxs, radius)
...
if __name__ == "__main__":
    ...
    diff1, diff2, diff3, out_list, center, radius = runTexture(images, alpha)
    [x, y] = center
    ...
    cv2.imwrite('diff3.png', diff3)
    img = cv2.imread('diff3.png')
    print('image shape =' + str(img.shape))
    print('center' +str(center))
    print('radius' +str(radius))
    cv2.circle(img, (x, y), 5, (0,255,0),2)
    cv2.imwrite('diff32.png', img)
